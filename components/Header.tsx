...
const Header: React.FC = () => {
  ...
  return (
    ...
    >
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <a
              ...
              className={`text-sm font-medium tracking-wide uppercase transition-colors hover:text-gold-500 relative group ${
                isScrolled ? 'text-slate-200' : 'text-white/90'
              }`}
            >
              {link.name}
          ...
          <a
            ...
            className={`cursor-pointer border px-5 py-2 text-sm uppercase tracking-wider font-semibold transition-all duration-300 ${
              isScrolled 
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-navy-950 z-40 flex flex-col items-center justify-center gap-8 transition-transform duration-300 md:hidden ${
